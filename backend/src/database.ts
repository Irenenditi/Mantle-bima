...
import type { Listing, ListingInput, StatusUpdate, Image, Document } from "./types";
...
export const dbHelpers = {
    ...
    return listings.map((listing) => {
      ...
      return {
        ...
        verificationStatus: verificationStatus
          ? {
              documents: verificationStatus.documents as "pending" | "approved" | "rejected",
              site: verificationStatus.site as "pending" | "approved" | "rejected",
              legal: verificationStatus.legal as "pending" | "approved" | "rejected",
            }
  ...
  getListingById: (id: string): Listing | null => {
    const listing = db.prepare("SELECT * FROM listings WHERE id = ?").get(id) as Listing | undefined;
    ...
    const utilities = db
      .prepare("SELECT utility FROM utilities WHERE listingId = ?")
      .all(id) as Array<{ utility: string }>;
    const amenities = db
      .prepare("SELECT amenity FROM amenities WHERE listingId = ?")
      .all(id) as Array<{ amenity: string }>;
    const verificationStatus = db
      .prepare("SELECT * FROM verification_status WHERE listingId = ?")
      .get(id) as {
      documents: string;
  ...
  // Create new listing
  createListing: (listingData: ListingInput & { id: string; createdAt: string; updatedAt: string; status: string }): Listing => {
    const transaction = db.transaction(() => {
      ...
      insertListing.run(
        ...
        listingData.landType || null,
        listingData.zoning || null,
        listingData.accessibility || null,
        listingData.metadataHash || null,
        listingData.status,
        listingData.sellerName || null,
        listingData.sellerPhone || null,
        listingData.sellerEmail || null,
    ...
    transaction();
    const createdListing = dbHelpers.getListingById(listingData.id);
    if (!createdListing) {
  ...
  updateListingStatus: (id: string, statusData: StatusUpdate): Listing => {
    const transaction = db.transaction(() => {
    ...
    transaction();
    const updatedListing = dbHelpers.getListingById(id);
    if (!updatedListing) {
  ...
  // Delete listing
  deleteListing: (id: string): Listing | null => {
...
// Database instance is internal to this module
// Use dbHelpers for all database operations
